...
test.describe("Public Homepage - Unauthenticated Access", () => {
  test("should load homepage without authentication", async ({
    authenticatedAdmin,
  }) => {
  ...
  test("should display quick stats cards", async ({ authenticatedAdmin }) => {
    ...
    await expect(page.locator("text=ครูผู้สอนทั้งหมด")).toBeVisible({
      timeout: 10000,
    });
  ...
  test("should display mini charts section", async ({ authenticatedAdmin }) => {
    ...
    const charts = await page
      .locator('.recharts-wrapper, [role="img"]')
      .count();
...
test.describe("Tab Navigation", () => {
  test("should switch between teachers and classes tabs", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/");
  ...
  test("should reset to page 1 when switching tabs", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
...
test.describe("Pagination", () => {
  test("should paginate through teachers", async ({ authenticatedAdmin }) => {
    ...
    const nextButton = page
      .locator('button:has-text("Next"), button[aria-label="Next page"]')
      .first();
...
test.describe("Table Interactions", () => {
  test("should have clickable view schedule links", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/?tab=teachers");
    ...
    const viewLinks = page.locator(
      'a:has-text("View schedule"), a:has-text("ดูตารางสอน")',
    );
  ...
  test("should have clickable view timetable links for classes", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/?tab=classes");
    ...
    const viewLinks = page.locator(
      'a:has-text("View timetable"), a:has-text("ดูตารางเรียน")',
    );
    await expect(viewLinks.first()).toBeVisible();
  ...
  test("should display teacher utilization badges", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/?tab=teachers");
  ...
  test("should display subject count badges", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/?tab=classes");
...
test.describe("Data Privacy & Security", () => {
  test("should not expose email addresses in HTML", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/?tab=teachers");
  ...
  test("should not expose email addresses in network responses", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
