...
test.describe("Teacher Arrange - Store Migration E2E", () => {
  ...
  test.describe("Teacher Selection & State Management", () => {
    test("should load teacher list and select a teacher", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Check if teacher selection dropdown/list exists
      const teacherSelect = page
        .getByRole("combobox", { name: /เลือกครู|teacher/i })
        .first();
    ...
    test("should persist selected teacher on page reload", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select a teacher
  ...
  test.describe("Subject Data Management", () => {
    test("should display subject list for selected teacher", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select teacher
      ...
      const subjectElements = page.locator(
        '[data-testid*="subject"], .subject-card, .subject-item',
      );
    ...
    test("should filter subjects when using search/filter", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select teacher
      ...
      const filterInput = page
        .locator(
          'input[placeholder*="ค้นหา"], select[name*="grade"], select[name*="filter"]',
        )
        .first();
  ...
  test.describe("Timeslot Operations", () => {
    test("should display timeslot grid", async ({ authenticatedAdmin }) => {
      ...
      const timeslotGrid = page.locator(
        '[data-testid*="timeslot"], .timeslot-grid, table',
      );
    ...
    test("should highlight timeslot on hover", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select teacher
      ...
      const firstTimeslot = page
        .locator('[data-testid*="timeslot"], .timeslot-cell, td')
        .first();
  ...
  test.describe("Drag and Drop Functionality", () => {
    test("should select subject when clicked", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select teacher
      ...
      const firstSubject = page
        .locator('[data-testid*="subject"], .subject-card, .subject-item')
        .first();
    ...
    test("should show error when trying invalid assignment", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select teacher
      ...
      const subject = page
        .locator('[data-testid*="subject"], .subject-card')
        .first();
      ...
      const lockedSlot = page
        .locator('[data-locked="true"], .timeslot-locked')
        .first();
      ...
      if (await lockedSlot.isVisible()) {
        ...
        const errorMsg = page.locator(
          "text=/ขัดแย้ง|ไม่สามารถ|conflict|error/i",
        );
  ...
  test.describe("Modal Operations", () => {
    test("should open room selection modal when assigning to timeslot", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select teacher
      ...
      const emptySlot = page
        .locator('[data-testid*="timeslot"]:not([data-has-subject="true"])')
        .first();
  ...
  test.describe("Save & Persistence", () => {
    test("should show saving indicator when saving schedule", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select teacher
      ...
      const savingIndicator = page.locator(
        "text=/กำลังบันทึก|Saving|Loading/i",
      );
  ...
  test.describe("Performance & Re-render Validation", () => {
    test("should maintain responsive UI during multiple state changes", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select teacher
    ...
    test("should handle localStorage persistence correctly", async ({
      authenticatedAdmin,
    }) => {
      const { page } = authenticatedAdmin;
      // Select teacher
        ...
        () => {
          return (
            document.querySelectorAll(
              '[data-testid*="subject"], .subject-card, .timeslot-grid, table',
            ).length > 0
          );
        },
        { timeout: 5000 },
