...
test.describe("Dashboard Page", () => {
  ...
  test("should redirect to semester selection when no semester provided", async ({
    authenticatedAdmin,
  }) => {
    ...
    await expect(page).toHaveURL(
      /\/(dashboard\/select-semester|dashboard\/\d-\d{4})/,
    );
  });

  test("should load dashboard with valid semester", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    ...
    await expect(
      page.locator("h1, h2").filter({ hasText: /ภาคเรียนที่/ }),
    ).toBeVisible();
  ...
  test("should display stat card values as numbers", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
  ...
  test("should display all 4 quick action buttons", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
    ...
    await expect(
      page.locator("a").filter({ hasText: "ตารางครู" }),
    ).toBeVisible();
    await expect(
      page.locator("a").filter({ hasText: "ตารางนักเรียน" }),
    ).toBeVisible();
    await expect(
      page.locator("a").filter({ hasText: "ตารางเวลา" }),
    ).toBeVisible();
    await expect(
      page.locator("a").filter({ hasText: "หลักสูตร" }),
    ).toBeVisible();
  });

  test("should navigate to teacher table when clicking quick action", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
  ...
  test("should navigate to student table when clicking quick action", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
  ...
  test("should display teacher workload chart", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
    ...
    const hasTeacherRows = await page
      .locator("text=/ชั่วโมง|ไม่พบข้อมูล/")
      .isVisible();
    expect(hasTeacherRows).toBeTruthy();
  });

  test("should display subject distribution chart", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
    ...
    const hasSubjectRows = await page
      .locator("text=/ชั่วโมง|ไม่พบข้อมูล/")
      .isVisible();
    expect(hasSubjectRows).toBeTruthy();
  });

  test("should show health indicators when there are issues", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
    ...
    if (isVisible) {
      ...
      const indicators = page.locator(
        "[class*='bg-yellow-50'], [class*='bg-red-50']",
      );
  ...
  test("should display summary info section", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
  ...
  test("should handle empty semester gracefully", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
  ...
  test("should display semester badge", async ({ authenticatedAdmin }) => {
    ...
    const badgeText = await page
      .locator("[class*='bg-blue'], [class*='bg-green']")
      .filter({ hasText: /ภาคเรียนที่/ })
      .textContent();
  ...
  test("should handle invalid semester format", async ({
    ...
    const hasError = await page
      .locator("text=/error|ข้อผิดพลาด|ไม่พบ/i")
      .isVisible();
  ...
  test("should display utilization bars in teacher workload", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
    ...
    const hasTeachers = await page
      .locator("text=/\\d+\\s*ชั่วโมง/")
      .isVisible();
  ...
  test("should display percentage bars in subject distribution", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567");
