...
test.describe("PDF Customization - Teacher Table", () => {
  ...
  test("TC-099-04: Test all paper sizes for teacher export", async ({ authenticatedAdmin }) => {
    ...
    for (const size of paperSizes) {
      ...
      // Take screenshot
      await pdfDialogPO.takeScreenshot(
        `pdf-dialog-teacher-${size.toLowerCase()}`,
      );
...
test.describe("PDF Customization - Student Table", () => {
  ...
  test("TC-099-14: Test toggle switches for students", async ({ authenticatedAdmin }) => {
    ...
    await pdfDialogPO.togglePageNumbers(false);
    await pdfDialogPO.takeScreenshot(
      "pdf-dialog-student-no-numbers-no-signatures",
    );
...
test.describe("PDF Customization - Cross-functionality", () => {
  ...
  test("TC-099-16: Export with all options enabled", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const teacherTablePO = new TeacherTablePO(page);
  ...
  test("TC-099-17: Export with all options disabled/minimum", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const studentTablePO = new StudentTablePO(page);
