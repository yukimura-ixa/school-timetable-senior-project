...
test.describe("Timetable Arrangement - Teacher View", () => {
  ...
  test("TC-010-01: Teacher arrangement page loads", async ({
    authenticatedAdmin,
  }) => {
    ...
    try {
      ...
      await expect(page.locator('main, table, [role="grid"]')).toBeVisible({
        timeout: 10000,
      });
  ...
  test("TC-010-02: Timetable grid structure", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const sampleSemester = "1-2567";
    ...
    try {
      ...
      const grid = page
        .locator('table, [role="grid"], .timetable-grid')
        .first();
  ...
  test("TC-010-03: Subject drag items visible", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const sampleSemester = "1-2567";
    ...
    try {
      ...
      const draggableItems = page.locator(
        '[draggable="true"], .draggable, [data-draggable]',
      );
...
test.describe("Timetable Arrangement - Student View", () => {
  ...
  test("TC-014-01: Student arrangement page loads", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const sampleSemester = "1-2567";
  ...
  test("TC-014-02: Class selection interface", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const sampleSemester = "1-2567";
...
test.describe("Lock Timeslots", () => {
  ...
  test("TC-015-01: Lock timeslots page loads", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const sampleSemester = "1-2567";
  ...
  test("TC-015-02: Lock interface elements", async ({ authenticatedAdmin }) => {
    ...
    try {
      ...
      const lockButtons = page.locator(
        'button:has-text("lock"), button:has-text("ล็อก")',
      );
