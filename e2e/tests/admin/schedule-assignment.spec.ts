...
test.describe('Admin: Schedule Assignment - Basic Operations', () => {
  ...
  test('should display available subjects for selected teacher', async ({ arrangePage }) => {
    // Arrange & Act
    const teacherName = getTeacherName(testTeacher.TeacherID);
    await arrangePage.selectTeacher(teacherName);
  ...
  test('should allow removing subject from timeslot', async ({ arrangePage }) => {
...
test.describe('Admin: Schedule Assignment - Cross-Semester Navigation', () => {
  ...
  test('should detect room double-booking conflict', async ({ arrangePage }) => {
  ...
  test('should prevent assignment to locked timeslot', async ({ arrangePage }) => {
  ...
  test('should prevent assignment during break time', async ({ arrangePage }) => {
    // Arrange
    const teacherName = getTeacherName(1);
    await arrangePage.selectTeacher(teacherName);
...
test.describe('Admin: Schedule Assignment - Timeslot Locking', () => {
  ...
  test('should allow assignment after unlocking timeslot', async ({ arrangePage }) => {
    // Arrange
    const { row, col } = getTimeslotCoords('MON', 1);
    await arrangePage.lockTimeslot(row, col);
    await arrangePage.unlockTimeslot(row, col);
...
test.describe('Admin: Schedule Assignment - Export Functionality', () => {
  ...
  test('should export schedule with multiple assignments', async ({ arrangePage, page }) => {
...
test.describe('Admin: Schedule Assignment - Cross-Semester Navigation', () => {
  ...
  test('should maintain schedule data per semester', async ({ arrangePage }) => {
