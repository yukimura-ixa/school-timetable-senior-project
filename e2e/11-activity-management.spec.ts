...
test.describe('Activity Management - CRUD Operations', () => {
  ...
  test('TC-ACT-001: Create new activity subject', async ({ authenticatedAdmin }) => {
    ...
    await test.step('Fill in activity details', async () => {
      ...
      await page.getByRole('option', { name: new RegExp(TEST_ACTIVITY.type, 'i') }).click();
    ...
    await test.step('Submit and verify creation', async () => {
      ...
      const submitButton = page.getByRole('button', { name: /create|save|บันทึก/i }).last();
  ...
  test('TC-ACT-002: Edit existing activity', async ({ authenticatedAdmin }) => {
    ...
    await test.step('Open edit modal', async () => {
      ...
      const activityRow = page.locator('tr', { has: page.getByText('ACT-E2E-002') });
      ...
      await expect(modal).toBeVisible();
      await expect(page.getByLabel(/subject.*code/i)).toHaveValue('ACT-E2E-002');
    });
    ...
    await test.step('Submit and verify update', async () => {
      const submitButton = page.getByRole('button', { name: /update|save/i }).last();
      await submitButton.click();
  ...
  test('TC-ACT-003: Delete activity with confirmation', async ({ authenticatedAdmin }) => {
    ...
    await test.step('Click delete button', async () => {
      const activityRow = page.locator('tr', { has: page.getByText('ACT-E2E-003') });
      const deleteButton = activityRow.getByRole('button', { name: /delete/i });
      await deleteButton.click();

      // Confirmation dialog should appear
      const confirmDialog = page.locator('[role="dialog"]', { has: page.getByText(/confirm|delete/i) });
      await expect(confirmDialog).toBeVisible();
    });

    await test.step('Confirm deletion', async () => {
      // Click confirm button
      const confirmButton = page.getByRole('button', { name: /delete|confirm|ลบ/i }).last();
  ...
  test('TC-ACT-004: Cancel deletion', async ({ authenticatedAdmin }) => {
    ...
    await test.step('Open and cancel deletion', async () => {
      const activityRow = page.locator('tr', { has: page.getByText('ACT-E2E-004') });
      const deleteButton = activityRow.getByRole('button', { name: /delete/i });
      await deleteButton.click();

      // Cancel deletion
      const cancelButton = page.getByRole('button', { name: /cancel|ยกเลิก/i }).last();
  ...
  test('TC-ACT-006: Test all activity types', async ({ authenticatedAdmin }) => {
    ...
    const activityTypes = [
      ...
      { code: 'ACT-SOCIAL', name: 'Test Social Service', type: 'SOCIAL_SERVICE' },
    ];
    ...
    for (const activity of activityTypes) {
      await test.step(`Create ${activity.type} activity`, async () => {
        ...
        await page.getByLabel(/activity.*type/i).click();
        await page.getByRole('option', { name: new RegExp(activity.type, 'i') }).click();
  ...
  test('TC-ACT-007: Table refresh after operations', async ({ authenticatedAdmin }) => {
    ...
    await test.step('Delete activity and verify count decrease', async () => {
      const activityRow = page.locator('tr', { has: page.getByText('ACT-REFRESH') });
      const deleteButton = activityRow.getByRole('button', { name: /delete/i });
      await deleteButton.click();
