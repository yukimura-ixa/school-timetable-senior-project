...
test.describe('Public Teacher Schedule Page', () => {
  test('should load teacher schedule without authentication', async ({ page }) => {
    ...
    const teacherName = page.locator('[data-testid="teacher-name"], p.font-medium');
  ...
  test('should display period numbers and time ranges', async ({ page }) => {
    ...
    const firstPeriodCell = page.locator('td').filter({ hasText: /คาบ 1/ }).first();
  ...
  test('should display subject details in schedule cells', async ({ page }) => {
    ...
    if (cellsWithData > 0) {
      ...
      await expect(page.locator('td .text-xs').first()).toContainText(/[ก-ฮ]\d{5}|[A-Z]{2}\d{3}/);
      
      // Should show grade level (e.g., "ม.1/1")
      await expect(page.locator('td .text-xs').filter({ hasText: /ม\.\d+\/\d+/ })).toHaveCount({ min: 1 });
    }
  });
  ...
  test('should handle no schedule data gracefully', async ({ page }) => {
    ...
    const noDataMessage = page.locator('text=/ไม่มีตารางสอน|ไม่พบข้อมูล|No data|not found/i');
    const isVisible = await noDataMessage.isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should navigate back to homepage', async ({ page }) => {
    ...
    const backButton = page.locator('a[href="/"], a:has-text("กลับหน้าแรก"), a:has-text("Back")');
...
test.describe('Public Class Schedule Page', () => {
  ...
  test('should display teacher names in schedule cells', async ({ page }) => {
    ...
    const teacherNames = page.locator('td').filter({ hasText: /นาย|นาง|นางสาว|Mr\.|Mrs\.|Ms\./ });
    ...
    if (count > 0) {
      ...
      await expect(teacherNames.first()).toContainText(/^(นาย|นาง|นางสาว|Mr\.|Mrs\.|Ms\.)\s+\S+\s+\S+/);
    }
  });
  ...
  test('should show program information if available', async ({ page }) => {
    ...
    const hasProgram = await programInfo.isVisible({ timeout: 3000 }).catch(() => false);
...
test.describe('Public Schedule Pages - Common Features', () => {
  ...
  test('should be responsive on mobile viewport', async ({ page }) => {
    ...
    const scrollContainer = page.locator('.overflow-x-auto, .overflow-auto').first();
...
test.describe('Public Schedule Pages - Error Handling', () => {
  ...
  test('should handle missing semester parameter', async ({ page }) => {
    ...
    if (response) {
      ...
      const isError = page.url().includes('404') || page.url().includes('error');
      const hasErrorMessage = await page.locator('text=/error|not found|ไม่พบ/i').isVisible().catch(() => false);
