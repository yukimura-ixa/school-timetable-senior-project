...
test.describe("Conflict Detector", () => {
  ...
  test("should navigate to conflicts page from dashboard", async ({ authenticatedAdmin }) => {
  ...
  test("should display conflict detector page components", async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test("should display summary cards with conflict counts", async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test("should switch between conflict tabs", async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test("should display conflict details when conflicts exist", async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin

    await page.goto("/dashboard/1-2567/conflicts")

    // ✅ Web-first: Wait for content to render (either table or empty state)
    const conflictContent = page.locator("table, [class*='conflict'], [class*='empty']")
  ...
  test("should display Thai labels correctly", async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test("should handle empty conflict state", async ({ authenticatedAdmin }) => {
    ...
    for (const tabName of tabs) {
      ...
      const content = page.locator("table, [class*='empty'], text=/ไม่พบ|ไม่มี/")
      await expect(content.first()).toBeVisible()
    }
  })

  test("should refresh data when navigating back", async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test("should maintain selected tab on page reload", async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
...
test.describe("Conflict Detector - Error Handling", () => {
  test("should handle network errors gracefully", async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test("should handle invalid semester parameter", async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
