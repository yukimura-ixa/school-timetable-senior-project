...
test.describe('TC-011: Teacher Double Booking Prevention', () => {
    ...
    test('TC-011-01: Teacher arrange page loads', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin

        await page.goto(`http://localhost:3000/schedule/${testSemester}/arrange/teacher-arrange`)
    ...
    test('TC-011-02: Cannot assign teacher to conflicting timeslot', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        if (await teacherSelector.isVisible()) {
            ...
            const timetableGrid = page.locator('table, [role="grid"], [class*="timetable"]')
    ...
    test('TC-011-03: System shows conflict error for teacher', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const errorIndicators = page.locator('text=/ซ้ำซ้อน|conflict|ไม่สามารถ|cannot/i')

        // Log if any conflict indicators are currently visible
        const hasVisibleError = await errorIndicators.first().isVisible().catch(() => false)
        console.log(`Conflict indicators visible: ${hasVisibleError}`)
    })

    test('TC-011-04: Conflicting timeslot is highlighted', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const conflictMarkers = page.locator('[class*="conflict"], [class*="error"], [class*="warning"]')
...
test.describe('TC-012: Class Double Booking Prevention', () => {
    ...
    test('TC-012-01: Student arrange page loads', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin

        await page.goto(`http://localhost:3000/schedule/${testSemester}/arrange/student-arrange`)
        await expect(page.locator('main, body')).toBeVisible({ timeout: 10000 })
    ...
    test('TC-012-02: Cannot assign multiple subjects to same class timeslot', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
    ...
    test('TC-012-03: System prevents class scheduling conflict', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
    ...
    test('TC-012-04: Existing class schedule blocks new assignment', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const occupiedSlots = page.locator('[class*="occupied"], [class*="filled"], [data-scheduled="true"]')
...
test.describe('TC-013: Room Availability Conflict Prevention', () => {
    ...
    test('TC-013-01: Room assignment interface accessible', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const hasRoomUI = await roomControls.first().isVisible().catch(() => false)
    ...
    test('TC-013-02: Cannot assign occupied room to another class', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
    ...
    test('TC-013-03: System shows room conflict error', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const roomConflictMsg = page.locator('text=/ห้องซ้ำ|room.*conflict|ห้อง.*ไม่ว่าง/i')
        const hasRoomConflictUI = await roomConflictMsg.first().isVisible().catch(() => false)

        console.log(`Room conflict messaging present: ${hasRoomConflictUI}`)
    })

    test('TC-013-04: Suggest alternative rooms when conflict detected', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const suggestionUI = page.locator('text=/แนะนำ|suggest|ห้องอื่น|available/i')
        const hasSuggestions = await suggestionUI.first().isVisible().catch(() => false)
...
test.describe('TC-011-013: Integrated Conflict Prevention', () => {
    ...
    test('TC-013-05: All conflict types are checked simultaneously', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
    ...
    test('TC-013-06: Conflict detection persists after page reload', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
    ...
    test('TC-013-07: Navigate to conflict report from arrange page', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const conflictLink = page.locator('a, button').filter({ hasText: /ตรวจสอบ.*ซ้ำ|conflict|view.*conflict/i })
        ...
        if (await conflictLink.first().isVisible()) {
            ...
            console.log('Direct conflict report link not found - may need to navigate via menu')
        }
    })
})
