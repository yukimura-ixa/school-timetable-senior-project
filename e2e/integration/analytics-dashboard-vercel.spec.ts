...
test.describe("Analytics Dashboard - Vercel Integration", () => {
  ...
  test.describe("Dashboard Visibility", () => {
    test("should display analytics dashboard when semesters exist", async ({
      ...
      const semesterCount = await page
        .locator('[data-testid="semester-card"], [class*="semester"]')
        .count();
  ...
  test.describe("Dashboard Statistics (if visible)", () => {
    test("statistics should be valid numbers", async ({ page }) => {
      ...
      const dashboardVisible = await page
        .locator("text=/📊.*แดชบอร์ด/i")
        .isVisible()
        .catch(() => false);

      test.skip(
        !dashboardVisible,
        "Dashboard not visible - likely no semesters",
      );

      // Find all numbers in dashboard
      const statsSection = page.locator("text=/📊.*แดชบอร์ด/i").locator("..");
      const numbers = await statsSection
        .locator("text=/\\d+/")
        .allTextContents();
  ...
  test.describe("API Integration", () => {
    test("should handle slow API responses gracefully", async ({ page }) => {
      ...
      const loadingVisible = await page
        .locator('[class*="Skeleton"], text=/loading|โหลด/i')
        .isVisible()
        .catch(() => false);
    ...
    test("should handle API errors gracefully", async ({ page }) => {
      ...
      const hasErrorOrEmpty = await page
        .locator("text=/error|ข้อผิดพลาด|empty|ไม่พบข้อมูล/i")
        .count();
  ...
  test.describe("Data Validation", () => {
    test("semester data should be valid if present", async ({ page }) => {
      ...
      for (const card of semesterCards.slice(0, 5)) {
        // Check first 5
