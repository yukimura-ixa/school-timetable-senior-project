...
test.describe('Security: PII Exposure Prevention', () => {
  ...
  test('TC-SEC-002: Teacher list page should not expose email addresses', async ({ authenticatedAdmin }) => {
    ...
    if (count > 0) {
      console.log(`✓ Found ${count} teacher cards (emails should not be visible)`);
    }
  });
  ...
  test('TC-SEC-004: API responses should not include email in public endpoints', async ({ page, request }) => {
  ...
  test('TC-SEC-005: Network requests should not leak sensitive data', async ({ authenticatedAdmin }) => {
    ...
    if (sensitiveDataFound.length > 0) {
      console.error('❌ Sensitive data found in network requests:', sensitiveDataFound);
    }
  });
});
