...
test.describe('Semester Smoke Tests - Schedule Config', () => {
  for (const term of SEEDED_TERMS) {
    ...
    test(`/schedule/${term.label}/config renders teacher table`, async ({ page }) => {
    ...
    test(`/schedule/${term.label}/config has metric cards`, async ({ page }) => {
      await page.goto(`/schedule/${term.label}/config`);
    ...
    test(`/schedule/${term.label}/config has no console errors`, async ({ page }) => {
...
test.describe('Semester Smoke Tests - Dashboard All-Timeslot', () => {
  for (const term of SEEDED_TERMS) {
    test(`/dashboard/${term.label}/all-timeslot returns 200 OK`, async ({ page }) => {
      const response = await page.goto(`/dashboard/${term.label}/all-timeslot`);
      expect(response?.status()).toBe(200);
    });

    test(`/dashboard/${term.label}/all-timeslot renders grade table`, async ({ page }) => {
    ...
    test(`/dashboard/${term.label}/all-timeslot has pagination`, async ({ page }) => {
      await page.goto(`/dashboard/${term.label}/all-timeslot`);
      
      // Check for pagination controls
      const pagination = page.locator('[role="navigation"]').filter({ hasText: /หน้า|Page/i });
      await expect(pagination).toBeVisible({ timeout: 10000 });
    });

    test(`/dashboard/${term.label}/all-timeslot has no console errors`, async ({ page }) => {
      const consoleErrors: string[] = [];
      page.on('console', (msg) => {
        if (msg.type() === 'error') {
          consoleErrors.push(msg.text());
