...
test.describe('Publish Gate', () => {
  test('should prevent publishing of an incomplete semester', async ({ page }) => {
    ...
    // 2. Try to publish
    const statusButton = page.locator('[data-testid="config-status-badge"]').getByRole('button').first();
    await statusButton.click();
    
    const publishOption = page.locator('role=menuitem').filter({ hasText: /เผยแพร่|PUBLISHED/i }).first();
