...
test.describe('Drag and Drop - Subject List to Timeslot', () => {
  ...
  test('TC-DND-001-01: Subject items are draggable', async ({ authenticatedAdmin }) => {
  ...
  test('TC-DND-001-02: Subject selection via click', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    const subjectItems = page.locator('[data-sortable-id*="SubjectCode"]').or(
      page.locator('.subject-item, [class*="subject"]').filter({ has: page.locator('b') })
    )
  ...
  test('TC-DND-001-03: Drag subject to empty timeslot', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test('TC-DND-001-04: Visual feedback during drag', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    const subjectItems = page.locator('[data-sortable-id*="SubjectCode"]').or(
...
test.describe('Drag and Drop - Between Timeslots', () => {
  ...
  test('TC-DND-002-01: Identify filled timeslots', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test('TC-DND-002-02: Drag subject between timeslots', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test('TC-DND-002-03: Click to change timeslot mode', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
...
test.describe('Drag and Drop - Conflict Detection', () => {
  ...
  test('TC-DND-003-01: Detect error indicators', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test('TC-DND-003-02: Attempt invalid drop (same slot)', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test('TC-DND-003-03: Drop on occupied slot (conflict)', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
...
test.describe('Drag and Drop - Lock State Behavior', () => {
  ...
  test('TC-DND-004-01: Identify locked timeslots', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test('TC-DND-004-02: Attempt drop on locked slot', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    const subjects = page.locator('[data-sortable-id*="SubjectCode"]').first()
  ...
  test('TC-DND-004-03: Locked slots are not draggable', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    const lockedSlots = page.locator('td, .timeslot-cell').filter({
      has: page.locator('svg[data-testid*="Lock"]')
    })
...
test.describe('Drag and Drop - Student Arrange Page', () => {
  ...
  test('TC-DND-006-01: Student page drag functionality', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    await page.screenshot({
  ...
  test('TC-DND-006-02: Class selection affects drag behavior', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
...
test.describe('Drag and Drop - Performance & Edge Cases', () => {
  ...
  test('TC-DND-007-01: Multiple rapid drags', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test('TC-DND-007-02: Drag outside boundaries', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    const subjects = page.locator('[data-sortable-id*="SubjectCode"]').first()
    const count = await subjects.count()
