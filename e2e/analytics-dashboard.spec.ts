...
test.describe("Analytics Dashboard - Access & Navigation", () => {
  test("should navigate to analytics from dashboard", async ({
    ...
    await page
      .locator('[data-testid="semester-card"], .semester-card')
      .first()
      .click();
    ...
    const analyticsButton = page
      .locator(
        'a[href*="/analytics"], button:has-text("Analytics"), button:has-text("วิเคราะห์")',
      )
      .first();
    ...
    if (await analyticsButton.isVisible({ timeout: 3000 })) {
      ...
      await expect(
        page.locator("h1, h2").filter({ hasText: /Analytics|วิเคราะห์|สถิติ/ }),
      ).toBeVisible();
    }
  });
  ...
  test("should show analytics page header", async ({ authenticatedAdmin }) => {
    ...
    ).toContainText(/Analytics|วิเคราะห์|Dashboard|แดชบอร์ด/);
  });
});

test.describe("Analytics Dashboard - Overview Stats Cards", () => {
  test("should display 4 overview stat cards", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const statCards = page.locator(
      '[data-testid*="stat-card"], .stat-card, [role="region"]',
    );
  ...
  test("should show total scheduled hours", async ({ authenticatedAdmin }) => {
    ...
    const scheduledHoursStat = page.locator(
      "text=/Total.*Hours|ชั่วโมง.*ทั้งหมด|Scheduled/i",
    );
  ...
  test("should show completion rate percentage", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const completionRate = page.locator(
      "text=/Completion|ความสมบูรณ์|เสร็จสิ้น/i",
    );
  ...
  test("should show active teachers count", async ({ authenticatedAdmin }) => {
    ...
    const activeTeachers = page.locator(
      "text=/Active.*Teachers|ครูที่ใช้งาน|ครูที่สอน/i",
    );
    await expect(activeTeachers).toBeVisible({ timeout: 10000 });
  });

  test("should show conflicts count with color coding", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const conflictCard = page
      .locator('[data-testid="conflicts-stat"], .stat-card')
      .filter({ hasText: /Conflict/i })
      .first();
...
test.describe("Analytics Dashboard - Teacher Workload Analysis", () => {
  test("should display teacher workload section", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    await expect(
      page
        .locator('h2, h3, [data-testid="section-title"]')
        .filter({ hasText: /Teacher.*Workload|ภาระงาน.*ครู/i }),
    ).toBeVisible({ timeout: 10000 });
  });

  test("should list all teachers with workload data", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const teacherEntries = page.locator(
      '[data-testid="teacher-entry"], .teacher-entry, [data-testid^="teacher-"]',
    );
  ...
  test("should show teacher names and workload hours", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const firstTeacher = page
      .locator('[data-testid="teacher-entry"], .teacher-entry')
      .first();
  ...
  test("should display workload status indicators", async ({
    ...
    const statusChips = page.locator(
      '[data-testid="workload-status"], .status-chip, .badge, .chip',
    );
    const count = await statusChips.count();

    if (count > 0) {
      // Should have status text like "ใช้งานต่ำ", "เหมาะสม", "สูง", "สูงเกินไป"
      await expect(statusChips.first()).toContainText(
        /ใช้งานต่ำ|เหมาะสม|สูง|สูงเกินไป|Low|Optimal|High|Overloaded/i,
      );
  ...
  test("should display visual progress bars for workload", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const width = await firstBar.evaluate(
      (el) => window.getComputedStyle(el).width,
    );
    expect(width).not.toBe("0px");
  });

  test("should sort teachers by workload descending", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const teacherEntries = page.locator(
      '[data-testid="teacher-entry"], .teacher-entry',
    );
    const count = await teacherEntries.count();
  ...
  test("should handle teachers with zero workload", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const zeroWorkload = page.locator(
      "text=/0.*ชม\.|0.*hours?|ไม่ได้ใช้งาน|Not.*active/i",
    );
  ...
  test("should show MaxHours limit for each teacher", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
...
test.describe("Analytics Dashboard - Room Utilization Analysis", () => {
  test("should display room utilization section", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    await expect(
      page
        .locator('h2, h3, [data-testid="section-title"]')
        .filter({ hasText: /Room.*Utilization|การใช้.*ห้อง|ห้องเรียน/i }),
    ).toBeVisible({ timeout: 10000 });
  });

  test("should list all rooms with utilization data", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const roomEntries = page.locator(
      '[data-testid="room-entry"], .room-entry, [data-testid^="room-"]',
    );
  ...
  test("should show room names and IDs", async ({ authenticatedAdmin }) => {
    ...
    const firstRoom = page
      .locator('[data-testid="room-entry"], .room-entry')
      .first();
  ...
  test("should display occupancy rates as percentages", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
  ...
  test("should display occupancy status chips", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
    ...
    const statusChips = page
      .locator('[data-testid="occupancy-status"], .status-chip, .badge')
      .filter({
        hasText:
          /Overutilized|Optimal|Moderate|Underutilized|ใช้.*มาก|เหมาะสม|ปานกลาง|ใช้.*น้อย/i,
      });
    const count = await statusChips.count();
  ...
  test("should sort rooms by occupancy rate descending", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
  ...
  test("should show scheduled periods vs total periods", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
  ...
  test("should display visual progress bars for occupancy", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
...
test.describe("Analytics Dashboard - Data Accuracy", () => {
  test("should show realistic workload percentages (0-150%)", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
  ...
  test("should show non-negative counts", async ({ authenticatedAdmin }) => {
    ...
    const numbers = await page
      .locator('[data-testid*="count"], .stat-card text')
      .allTextContents();
  ...
  test("should calculate completion rate correctly", async ({
    ...
    const completionText = await page
      .locator('[data-testid="completion-rate"], text=/Completion.*\d+%/i')
      .first()
      .textContent();
...
test.describe("Analytics Dashboard - UI/UX", () => {
  test("should be scrollable for long lists", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
...
test.describe("Analytics Dashboard - Performance", () => {
  ...
  test("should handle large datasets (50+ teachers)", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    await page.goto("/dashboard/1-2567/analytics");
