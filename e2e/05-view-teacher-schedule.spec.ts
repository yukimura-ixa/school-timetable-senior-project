...
test.describe('TC-017: View Teacher Schedule - Admin Role', () => {
    const testSemester = '1-2567' // Semester 1, Year 2567

    test('TC-017-01: Admin navigates to teacher schedule page', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin

        await page.goto(`http://localhost:3000/dashboard/${testSemester}/teacher-table`)
    ...
    test('TC-017-02: Admin can select different teachers', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const teacherSelectorContainer = page.locator('div', { has: page.getByText('เลือกครู') }).first()

        // Check if the selector is interactive (not disabled via opacity/pointer-events)
        const opacity = await teacherSelectorContainer.evaluate(el =>
    ...
    test('TC-017-03: Schedule grid displays timeslots', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const scheduleGrid = page.locator('table, [role="grid"], [class*="timetable"], [class*="schedule"]')
        const hasGrid = await scheduleGrid.first().isVisible().catch(() => false)

        if (hasGrid) {
            console.log('Schedule grid is visible')

            // Look for timeslot cells
            const timeslots = page.locator('td, [class*="timeslot"], [class*="cell"]')
    ...
    test('TC-017-04: Schedule shows subject information', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const subjectInfo = page.locator('[class*="subject"], [class*="class"], td, div').filter({
            hasText: /ม\.\d|[A-Z]\d{5}|วิทย|คณิต|ภาษา/
        })

        const hasSubjects = await subjectInfo.first().isVisible().catch(() => false)
    ...
    test('TC-017-05: Export options are available for admin', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
    ...
    test('TC-017-06: Bulk export interface for multiple teachers', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const hasBulkUI = await bulkUI.first().isVisible().catch(() => false)
...
test.describe('TC-017: View Teacher Schedule - Teacher Role', () => {
    ...
    test('TC-017-07: Teacher sees own schedule automatically', async ({ page }) => {
        ...
        const devBypass = page.getByRole('button', { name: /dev.*bypass|bypass.*dev/i })
        const hasBypass = await devBypass.isVisible().catch(() => false)

        if (hasBypass) {
            console.log('Dev bypass available - teacher role test requires proper auth setup')
    ...
    test('TC-017-08: Teacher selector is disabled for non-admin', async ({ page }) => {
        ...
        const teacherSelector = page.locator('div', { has: page.getByText('เลือกครู') }).first()
...
test.describe('TC-017: Schedule Display and Navigation', () => {
    ...
    test('TC-017-09: Schedule shows all days of week', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const days = ['จันทร์', 'อังคาร', 'พุธ', 'พฤหัสบดี', 'ศุกร์', 'MON', 'TUE', 'WED', 'THU', 'FRI']
    ...
    test('TC-017-10: Schedule shows time periods', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const timeElements = page.locator('th, td, div, span').filter({ hasText: timePattern })
    ...
    test('TC-017-11: Empty timeslots are clearly indicated', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const hasEmptyIndicators = await emptyIndicators.first().isVisible().catch(() => false)
    ...
    test('TC-017-12: Responsive design on mobile viewport', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
    ...
    test('TC-017-13: Print view is functional', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const printButton = page.locator('button').filter({ hasText: /print|พิมพ์/i })
...
test.describe('TC-017: Export Functionality', () => {
    ...
    test('TC-017-14: Excel export button is visible', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const excelButton = page.locator('button, a').filter({ hasText: /excel|xlsx|ส่งออก.*excel/i })
        const hasExcel = await excelButton.first().isVisible().catch(() => false)
    ...
    test('TC-017-15: PDF export button is visible', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const pdfButton = page.locator('button, a').filter({ hasText: /pdf|ส่งออก.*pdf/i })
        const hasPdf = await pdfButton.first().isVisible().catch(() => false)
    ...
    test('TC-017-16: Export menu opens on click', async ({ authenticatedAdmin }) => {
        const { page } = authenticatedAdmin
        ...
        const exportMenuButton = page.locator('button').filter({
            hasText: /export|ส่งออก|download|ดาวน์โหลด/i
        }).or(
            page.locator('button[aria-haspopup="true"]')
        )

        const hasMenu = await exportMenuButton.first().isVisible().catch(() => false)
