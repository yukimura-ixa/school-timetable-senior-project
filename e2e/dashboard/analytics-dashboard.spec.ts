...
test.describe("Analytics Dashboard", () => {
  ...
  test.describe("Dashboard Visibility", () => {
    test("should display dashboard header with emoji and title", async ({
      page,
    }) => {
    ...
    test("should show expand/collapse button", async ({ page }) => {
      ...
      const buttonExists = await toggleButton
        .isVisible()
        .catch(() => fallbackButton.isVisible());
  ...
  test.describe("Collapse/Expand Functionality", () => {
    test("should collapse when toggle button is clicked", async ({ page }) => {
      ...
      await expect(async () => {
        const isVisible = await statsBeforeCollapse
          .isVisible()
          .catch(() => false);
    ...
    test("should expand when toggle button is clicked again", async ({
      page,
    }) => {
      // Find toggle button
      const toggleButton = page
    ...
    test("should have smooth animation on collapse/expand", async ({
      page,
    }) => {
      const toggleButton = page
        .locator("text=/📊.*แดชบอร์ดวิเคราะห์/")
    ...
    test("should toggle icon between ExpandMore and ExpandLess", async ({
      page,
    }) => {
      const toggleButton = page
        .locator("text=/📊.*แดชบอร์ดวิเคราะห์/")
  ...
  test.describe("Overview Statistics Cards", () => {
    ...
    test("should show tooltips on hover (if implemented)", async ({ page }) => {
      ...
      await expect(async () => {
        ...
        console.log(
          `Tooltips ${tooltipExists ? "are" : "are not"} implemented`,
        );
      }).toPass({ timeout: 1000 });
    });

    test("average completeness should be a valid percentage", async ({
      page,
    }) => {
      const avgCompletenessCard = page
        .locator("text=/ความสมบูรณ์เฉลี่ย/")
        .locator("..");

      // Find the percentage value
      const percentageText = await avgCompletenessCard
        .locator("text=/%/")
        .textContent();
  ...
  test.describe("Status Distribution Section", () => {
    ...
    test("should display progress bars for each status", async ({ page }) => {
      ...
      const progressBars = page.locator(
        '[role="progressbar"], [class*="LinearProgress"], [class*="progress"]',
      );
    ...
    test("percentages should add up to approximately 100%", async ({
      page,
    }) => {
  ...
  test.describe("Completeness Distribution Section", () => {
    ...
    test("should use color coding (red/orange/green)", async ({ page }) => {
      ...
      const coloredElements = await completenessSection
        .locator('[style*="color"], [class*="color"]')
        .count();
  ...
  test.describe("Resource Totals Section", () => {
    ...
    test("should display numeric counts for each resource", async ({
      page,
    }) => {
    ...
    test("should have 4-column responsive layout on desktop", async ({
      page,
    }) => {
  ...
  test.describe("Academic Year Distribution Section", () => {
    ...
    test("should display progress bars for year distribution", async ({
      page,
    }) => {
      const yearSection = page
        .locator("text=/กระจายตามปีการศึกษา/")
  ...
  test.describe("Data Accuracy", () => {
    test("total semesters should match semester count", async ({ page }) => {
      ...
      if (totalMatch) {
        ...
        const semesterCards = await page
          .locator('[class*="semester"], [data-testid*="semester"]')
          .count();
    ...
    test("statistics should be non-negative", async ({ page }) => {
      ...
      const dashboardSection = page
        .locator("text=/📊.*แดชบอร์ดวิเคราะห์/")
        .locator("..");
      const numbers = await dashboardSection
        .locator("text=/\\d+/")
        .allTextContents();
  ...
  test.describe("Performance", () => {
    ...
    test("should not cause layout shifts", async ({ page }) => {
      ...
      await expect(async () => {
        ...
        expect(
          Math.abs(currentMetrics.scrollHeight - metrics1.scrollHeight),
        ).toBeLessThan(10);
      }).toPass({ timeout: 2000 });
      ...
      expect(
        Math.abs(metrics1.scrollHeight - metrics2.scrollHeight),
      ).toBeLessThan(50);
    });
  });
  ...
  test.describe("Edge Cases", () => {
    ...
    test("should handle extreme values (100% completeness)", async ({
      page,
    }) => {
