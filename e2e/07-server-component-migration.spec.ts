...
test.describe("Server Component Migration - Teacher Management", () => {
  ...
  test("TC-007-02: Teacher data appears in initial HTML (SSR verification)", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const nav = new NavigationHelper(page);
    ...
    page.on("response", async (response) => {
      if (
        response.url().includes("/management/teacher") &&
  ...
  test("TC-007-03: Client interactions still work (mutations)", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const nav = new NavigationHelper(page);
...
test.describe("Server Component Migration - Other Management Pages", () => {
  ...
  test("TC-007-05: Subjects page renders with server data", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const nav = new NavigationHelper(page);
  ...
  test("TC-007-06: GradeLevel page renders with server data", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const nav = new NavigationHelper(page);
...
test.describe("Server Component Migration - Performance", () => {
  ...
  test("TC-007-08: No SWR revalidation requests on mount", async ({
    ...
    const dataFetchRequests = apiRequests.filter(
      (req) =>
        req.includes("GET") &&
        (req.includes("/api/teacher") || req.includes("teacher?")),
    );
...
test.describe("Server Component Migration - Dashboard Header", () => {
  test("TC-007-09: Dashboard header renders without useParams client hook", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
...
test.describe("Server Component Migration - Regression Tests", () => {
  test("TC-007-10: All management pages still accessible", async ({
    ...
    const pages = [
      {
        name: "Teacher",
        navigate: () => nav.goToTeacherManagement(),
        url: "/management/teacher",
      },
      {
        name: "Rooms",
        navigate: () => nav.goToRoomManagement(),
        url: "/management/rooms",
      },
      {
        name: "Subject",
        navigate: () => nav.goToSubjectManagement(),
        url: "/management/subject",
      },
      {
        name: "GradeLevel",
        navigate: () => nav.goToGradeLevelManagement(),
  ...
  test("TC-007-11: Search functionality still works (client-side filtering)", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const nav = new NavigationHelper(page);
    ...
    const searchInput = page
      .locator(
        'input[type="text"], input[type="search"], input[placeholder*="ค้นหา"], input[placeholder*="Search"]',
      )
      .first();
  ...
  test("TC-007-12: Pagination still works (client-side)", async ({
    authenticatedAdmin,
  }) => {
    const { page } = authenticatedAdmin;
    const nav = new NavigationHelper(page);
    ...
    const nextButton = page
      .locator(
        'button:has-text("Next"), button:has-text("ถัดไป"), button[aria-label*="next"]',
      )
      .first();
    const pagination = page
      .locator('[role="navigation"], .pagination, nav')
      .first();
