...
test.describe("Issue #94 - Teacher Assignment Management", () => {
  test.describe("Navigation and Access Control", () => {
    ...
    test("should display teacher assignment card in management page", async ({
      page,
    }) => {
      ...
      const description = page.locator(
        "text=กำหนดครูผู้สอนแต่ละวิชาตามระดับชั้น",
      );
  ...
  test.describe("Filter Controls", () => {
    test.beforeEach(async ({ page }) => {
      ...
      await page.waitForSelector('h1:has-text("จัดการมอบหมายครูผู้สอน")', {
        timeout: 10000,
      });
  ...
  test.describe("Teacher Assignment Operations", () => {
    test.beforeEach(async ({ page }) => {
      ...
      // Wait for page title
      await expect(
        page.locator('h1:has-text("จัดการมอบหมายครูผู้สอน")'),
    ...
    test("should display subject table with correct columns", async ({
      page,
    }) => {
    ...
    test("should show teacher selector for unassigned subjects", async ({
      page,
    }) => {
      ...
      if ((await unassignedRow.count()) > 0) {
        ...
        const assignButton = unassignedRow.locator(
    ...
    test("should assign teacher to subject", async ({ page }) => {
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        const autocompleteCount = await row
    ...
    test("should unassign teacher from subject", async ({ page }) => {
      ...
      for (let i = 0; i < rowCount; i++) {
        const row = rows.nth(i);
        const deleteButtonCount = await row
          .locator('button[color="error"]')
          .count();
  ...
  test.describe("Bulk Operations", () => {
    ...
    test("should display bulk action buttons", async ({ page }) => {
      ...
      const copyButton = page.locator(
        'button:has-text("คัดลอกจากภาคเรียนก่อนหน้า")',
      );
      await expect(copyButton).toBeVisible();

      // Clear all button (actual text from component)
      const clearButton = page.locator(
        'button:has-text("ลบการมอบหมายทั้งหมด")',
      );
    ...
    test("should show confirmation dialog for clear all", async ({ page }) => {
      ...
      await expect(async () => {
        ...
        const hasButtons =
          (await confirmButton.isVisible()) || (await cancelButton.isVisible());
      ...
      if (hasDialog) {
        ...
        const cancelButton = page
          .locator('button:has-text("ยกเลิก")')
  ...
  test.describe("Error Handling", () => {
    ...
    test("should show validation error for overloaded teacher", async ({
      page,
    }) => {
  ...
  test.describe("Responsive Design", () => {
    test("should work on mobile viewport", async ({ page }) => {
      ...
      const gradeFilter = page
        .locator("#grade-select")
        .or(page.getByLabel("ระดับชั้น"));
