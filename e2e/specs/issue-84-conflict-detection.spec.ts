...
async function dragSubjectToTimeslot(
  ...
  const timeslotCell = page
    .locator(`[data-day="${dayIndex}"][data-period="${periodIndex}"]`)
    .first();
...
test.describe("Issue #84 - Conflict Detection", () => {
  ...
  test.describe("Teacher Conflict Detection", () => {
    test("should prevent dragging subject when teacher has conflict", async ({
      page,
    }) => {
    ...
    test("should allow dragging subject when teacher is free", async ({
      page,
    }) => {
      ...
      const errorSnackbar = page.locator(
        ".notistack-snackbar.notistack-variantError",
      );
      await expect(errorSnackbar).not.toBeVisible();
    });

    test("should show detailed conflict information in error message", async ({
      page,
    }) => {
      ...
      const errorMessage = page
        .locator(".notistack-snackbar.notistack-variantError")
        .first();
  ...
  test.describe("Room Conflict Detection", () => {
    ...
    test("should allow selecting available room", async ({ page }) => {
      ...
      const successSnackbar = page.locator(
        ".notistack-snackbar.notistack-variantSuccess",
      );
      ...
      const scheduleCell = page
        .locator('[data-day="1"][data-period="0"]')
        .first();
    ...
    test("should allow re-selecting different room after conflict", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator(".notistack-snackbar.notistack-variantError"),
      ).toBeVisible();
      await expect(page.locator("text=เลือกห้องเรียน")).toBeVisible();
  ...
  test.describe("Combined Conflict Scenarios", () => {
    test("should check teacher conflict before room conflict", async ({
      page,
    }) => {
      ...
      await expect(
        page.locator(".notistack-snackbar", { hasText: "สอนวิชา" }),
      ).toBeVisible();
    ...
    test("should handle locked timeslot before conflict checks", async ({
      page,
    }) => {
    ...
    test("should validate full placement flow: drag → teacher check → room check → success", async ({
      page,
    }) => {
      ...
      const scheduleCell = page
        .locator('[data-day="2"][data-period="0"]')
        .first();
  ...
  test.describe("UI/UX Behavior", () => {
    ...
    test("should show loading state during async conflict checks", async ({
      page,
    }) => {
