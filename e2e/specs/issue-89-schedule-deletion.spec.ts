...
test.describe("Issue #89 - Schedule Deletion", () => {
  ...
  test("should revalidate SWR cache after deletion", async ({
    arrangePage,
    page,
  }) => {
  ...
  test("should persist deletion across page reload", async ({
    arrangePage,
    page,
  }) => {
    // Delete schedule
  ...
  test("should handle deletion when schedule is already empty", async ({
    arrangePage,
  }) => {
  ...
  test("should update subject availability after deletion", async ({
    arrangePage,
  }) => {
    ...
    const isEmpty = await arrangePage
      .assertScheduleEmpty()
      .then(() => true)
      .catch(() => false);
