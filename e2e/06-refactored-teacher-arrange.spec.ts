...
test.describe('Refactored TeacherArrangePage - Core Functionality', () => {
  ...
  test('E2E-005: Drag and drop interaction (visual check)', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    await page.goto(`/schedule/${SEMESTER}/arrange/teacher-arrange?TeacherID=${TEACHER_ID}`)
  ...
  test('E2E-006: Room selection modal appears', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    await page.goto(`/schedule/${SEMESTER}/arrange/teacher-arrange?TeacherID=${TEACHER_ID}`)
  ...
  test('E2E-008: No critical Zustand store errors', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
  ...
  test('E2E-009: Redux DevTools integration', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    await page.goto(`/schedule/${SEMESTER}/arrange/teacher-arrange?TeacherID=${TEACHER_ID}`)
...
test.describe('Refactored TeacherArrangePage - Conflict Detection', () => {
  ...
  test('E2E-011: Locked timeslot indicators', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    await page.goto(`/schedule/${SEMESTER}/arrange/teacher-arrange?TeacherID=${TEACHER_ID}`)
  ...
  test('E2E-012: Break time slots styled correctly', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    await page.goto(`/schedule/${SEMESTER}/arrange/teacher-arrange?TeacherID=${TEACHER_ID}`)
  ...
  test('E2E-013: Conflict indicators appear', async ({ authenticatedAdmin }) => {
    const { page } = authenticatedAdmin
    await page.goto(`/schedule/${SEMESTER}/arrange/teacher-arrange?TeacherID=${TEACHER_ID}`)
