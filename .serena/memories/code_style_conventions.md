...
export const getTeacherByIdSchema = v.object({
  TeacherID: v.pipe(v.string(), v.minLength(1, "รหัสครูห้ามว่าง")),
});
...
export function useSemesterSync(configId: string) {
  const { semester, academicYear, setSemester, setAcademicYear } =
    useSemesterStore();
...
export const getDataAction = createAction(schema, async (input) => {
...
export function validateTimeslotNotOverlap(
  ...
  const overlap = timeslots.find(
    (t) =>
      t.Day === newTimeslot.Day && t.PeriodStart === newTimeslot.PeriodStart,
  );
...
describe("validateTeacherNotOverbooked", () => {
  ...
  it("should return invalid when teacher has scheduling conflict", () => {
    const result = validateTeacherNotOverbooked(
      mockTeacher,
...
const existingClass = assignments.find(
  (a) => a.Day === timeslot.Day && a.PeriodStart === timeslot.PeriodStart,
);
