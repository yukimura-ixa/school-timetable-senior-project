...
async function verifyProgramSeed() {
  ...
  // Group by Year (1..6)
  const groupedByYear = programs.reduce((acc: Record<number, program[]>, program: program) => {
  ...
  for (const [year, yearPrograms] of (Object.entries(groupedByYear) as [string, program[]][]).sort((a, b) => Number(a[0]) - Number(b[0]))) {
    console.warn(`\n📅 Year ม.${year} (${yearPrograms.length} programs)`)
    console.warn('-'.repeat(80))

    const groupedByTrack = yearPrograms.reduce((acc: Record<string, program[]>, p: program) => {
    ...
    for (const [track, list] of Object.entries(groupedByTrack)) {
      console.warn(`\n  📖 Track: ${track}`)
      list.forEach((p: any) => {
        console.warn(`    • ${p.ProgramCode.padEnd(14)} ${p.ProgramName.padEnd(40)} (${p._count?.gradelevel ?? 0} grade levels)`)
      })
    }
  }
