...
- **Steps to Reproduce:**  
  1. Go to `/schedule/2567/1/arrange`.  
  2. Select a teacher (example: `TeacherID=620`).  
  3. Observe the “เวลา” row.  
  4. Repeat on `/dashboard/2567/1/teacher-table` after selecting a teacher.  
...
- **Impact:** From `/dashboard/2567/1`, clicking sidebar navigation sends user to `/dashboard` (semester picker) instead of staying under the selected semester.  
- **Environment:** Production, admin session, semester `1-2567` on 2025-12-18.  
- **Steps to Reproduce:**  
  1. Open `/dashboard/2567/1`.  
  2. Click sidebar “แสดงตารางรวมครู”.  
  3. Observe URL becomes `/dashboard`.  
- **Expected:** `/dashboard/2567/1/all-timeslot`.  
- **Actual:** `/dashboard`.  
- **Evidence:**  
...
- **Files Changed:**
  ...
  - `src/app/(public)/classes/[gradeId]/[academicYear]/[semester]/page.tsx` - Updated to use only GradeID
