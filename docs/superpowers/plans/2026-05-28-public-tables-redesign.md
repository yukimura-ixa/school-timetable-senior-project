...
Run: `pnpm test src/lib/ui/monogram.test.ts`
Expected: FAIL — module not found.

- [ ] **Step 3: Implement monogram util**

```ts
// src/lib/ui/monogram.ts
/**
 * Thai-aware monogram extraction for avatar bubbles.
 * Strips common honorific prefixes, then returns the first grapheme cluster
 * of the remaining name. Falls back to the first grapheme of the raw input
 * when no prefix matches (covers English names too).
 */
const PREFIXES = [
  "นางสาว",
  "เด็กชาย",
...
export function extractMonogram(name: string): string {
  ...
  if (!trimmed) return "";
  // Intl.Segmenter handles Thai grapheme clusters correctly.
