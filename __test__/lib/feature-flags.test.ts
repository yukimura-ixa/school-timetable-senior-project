...
import { isFeatureEnabled, isValidFeatureFlag, type FlagConfig } from '@/lib/feature-flags';
...
describe('Feature Flags Service', () => {
  ...
  describe('isFeatureEnabled', () => {
    ...
    describe('Role-Based Access Control', () => {
      it('returns true when user role is in allowedRoles', async () => {
        ...
        const result = await isFeatureEnabled('analyticsV2', 'user123', 'admin');
      ...
      it('returns false when user role is not in allowedRoles', async () => {
        ...
        const result = await isFeatureEnabled('analyticsV2', 'user123', 'student');
      ...
      it('returns true when allowedRoles is empty array', async () => {
        ...
        const result = await isFeatureEnabled('newScheduleUI', 'user123', 'student');
  ...
  describe('Evaluation Order', () => {
    it('follows correct evaluation order: enabled → role → email → rollout', async () => {
      ...
      expect(await isFeatureEnabled('test', 'user1', 'admin', 'admin@school.ac.th')).toBe(false);

      // Test 2: wrong role stops at role check
      mockGet.mockResolvedValue(mockConfig);
      expect(await isFeatureEnabled('test', 'user1', 'student', 'admin@school.ac.th')).toBe(false);

      // Test 3: wrong email stops at email check
      mockGet.mockResolvedValue(mockConfig);
      expect(await isFeatureEnabled('test', 'user1', 'admin', 'wrong@email.com')).toBe(false);

      // Test 4: all checks pass, reaches rollout
      mockGet.mockResolvedValue({ ...mockConfig, rolloutPercent: 100 });
      expect(await isFeatureEnabled('test', 'user1', 'admin', 'admin@school.ac.th')).toBe(true);
    });
  });
});
