...
describe("MOE Validation Utilities", () => {
  ...
  describe("validateProgramStandards", () => {
    ...
    it("should detect missing core subjects", () => {
      const subjects = createValidM1Program().filter(
        (s) => s.subjectCode !== "TH",
      );
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.includes("ขาดวิชาแกนหลัก"))).toBe(
        true,
      );
    ...
    it("should detect too few total lessons", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.includes("ต่ำกว่ามาตรฐาน"))).toBe(
        true,
      );
    });
    ...
    it("should warn about insufficient core subject lessons", () => {
      ...
      expect(
        result.warnings.some(
          (w) => w.includes("ภาษาไทย") && w.includes("น้อยกว่า"),
        ),
      ).toBe(true);
    });
