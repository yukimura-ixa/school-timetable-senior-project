...
describe.skip("PDFCustomizationDialog", () => {
  ...
  describe("Dialog Rendering", () => {
    ...
    it("should render action buttons", () => {
      ...
      expect(
        screen.getByRole("button", { name: /รีเซ็ต/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /ยกเลิก/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /ส่งออก PDF/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("Default Values", () => {
    it("should initialize with default PDF options", () => {
      ...
      expect(paperSizeSelect).toHaveTextContent(
        PAPER_SIZE_LABELS[DEFAULT_PDF_OPTIONS.format!],
      );
    });
  ...
  describe("Paper Size Selection", () => {
    it("should render all paper size options", async () => {
      ...
      expect(
        screen.getByRole("option", { name: PAPER_SIZE_LABELS.a4 }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: PAPER_SIZE_LABELS.a3 }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: PAPER_SIZE_LABELS.letter }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: PAPER_SIZE_LABELS.legal }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: PAPER_SIZE_LABELS.tabloid }),
      ).toBeInTheDocument();
    });
    ...
    it("should update paper size when option is selected", async () => {
      ...
      const a3Option = screen.getByRole("option", {
        name: PAPER_SIZE_LABELS.a3,
      });
  ...
  describe("Orientation Selection", () => {
    it("should render portrait and landscape options", async () => {
      ...
      const orientationSelect = screen.getByRole("combobox", {
        name: /การวางแนว/i,
      });
      await user.click(orientationSelect);

      expect(
        screen.getByRole("option", { name: /แนวตั้ง/i }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("option", { name: /แนวนอน/i }),
      ).toBeInTheDocument();
    });
  ...
  describe("Toggle Switches", () => {
    it("should render page numbers switch in checked state by default", () => {
      ...
      const pageNumbersSwitchInput = getSwitchInputByTestId(
        "page-numbers-switch",
      );
  ...
  describe("Accessibility", () => {
    it("should have proper ARIA labels for form controls", () => {
      ...
      expect(screen.getByTestId("orientation-select")).toHaveAttribute(
        "aria-label",
      );
    });
  ...
  describe("Edge Cases", () => {
    it("should handle missing defaultValues gracefully", () => {
      render(
        <PDFCustomizationDialog {...defaultProps} defaultValues={undefined} />,
      );
    ...
    it("should handle maxTablesPerPage = 10", () => {
      render(
        <PDFCustomizationDialog {...defaultProps} maxTablesPerPage={10} />,
      );
