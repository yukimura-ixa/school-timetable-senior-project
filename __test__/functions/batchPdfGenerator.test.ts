...
describe('BatchPdfGenerator', () => {
  describe('Constants', () => {
    ...
    it('should have all paper sizes covered in labels', () => {
      const paperSizes: PaperSize[] = ['a4', 'a3', 'letter', 'legal', 'tabloid'];
  ...
  describe('generateBatchPDF', () => {
    ...
    it('should throw error when elements array is empty', async () => {
      await expect(
        generateBatchPDF([], mockLabels, {})
      ).rejects.toThrow('No elements provided for PDF generation');
    });
    ...
    it('should handle many elements', async () => {
      ...
      const manyLabels = Array(20).fill(0).map((_, i) => `Label ${i + 1}`);
  ...
  describe('generateTeacherBatchPDF', () => {
    ...
    it('should handle Thai characters in teacher names', async () => {
      const thaiNames = [
        'อาจารย์ภาษาไทย สระอะอา',
        'ครูวิชาการ เลขานุการ',
      ];
    ...
    it('should handle many teachers', async () => {
      ...
      const manyNames = Array(50).fill(0).map((_, i) => `ครูคนที่ ${i + 1}`);
  ...
  describe('generateStudentBatchPDF', () => {
    ...
    it('should handle different grade level formats', async () => {
      const mixedGrades = ['ม.1/1', 'ม.2/3', 'ม.3/5', 'ม.4/2', 'ม.5/1', 'ม.6/4'];
    ...
    it('should handle many classes', async () => {
      ...
      const manyLabels = Array(30).fill(0).map((_, i) => `ม.${Math.floor(i / 5) + 1}/${(i % 5) + 1}`);
  ...
  describe('Option Merging', () => {
    it('should merge custom options with defaults', async () => {
      ...
      const result = await generateBatchPDF(mockElements, mockLabels, customOptions);
      expect(result).toBe(true);
    });
    ...
    it('should handle undefined custom options', async () => {
      ...
      const result = await generateBatchPDF(mockElements, mockLabels, undefined);
      expect(result).toBe(true);
    });
  });
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle long header text', async () => {
      ...
      const result = await generateBatchPDF(mockElements, mockLabels, {
        headerText: 'This is a very long header text that might wrap to multiple lines in the PDF',
      });
