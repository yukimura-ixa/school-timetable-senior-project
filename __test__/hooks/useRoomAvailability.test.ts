...
describe('computeAvailability', () => {
  describe('with empty selections', () => {
    it('should return available for rooms with no locks when no timeslots selected', () => {
      const lockedSchedules: Array<Pick<class_schedule, 'RoomID' | 'TimeslotID'>> = [];
  ...
  describe('table-driven test suite', () => {
    const testCases = [
      ...
      {
        description: 'locks present, no selection → partial for all locked rooms',
        lockedSchedules: [
          { RoomID: 1, TimeslotID: '1-2567-1A' },
    ...
    testCases.forEach(({ description, lockedSchedules, selectedTimeslots, expected }) => {
      it(description, () => {
        const result = computeAvailability(
          lockedSchedules as Array<Pick<class_schedule, 'RoomID' | 'TimeslotID'>>,
