 ...
 * const { semester, academicYear, configId } = useSemesterSync(params.semesterAndyear);
 */
export function useSemesterSync(semesterAndYear: string | string[] | undefined): SemesterSyncResult {
  const { setSemester, selectedSemester } = useSemesterStore();

  // Normalize and parse the input safely
  const { semester, academicYear, configId, semesterNum, academicYearNum } = useMemo(() => {
    ...
    const valid = Number.isFinite(semNum) && Number.isFinite(yearNum) && semStr !== "" && yearStr !== "";
  ...
  useEffect(() => {
    ...
    if (configId && semesterNum && academicYearNum && selectedSemester !== configId) {
