...
export async function GET(request: NextRequest) {
    ...
    const [teacher_responsibility, teacher_responsibilityField] =
        await pool.promise()
        .query(`SELECT teacher.TeacherID, subject.SubjectID, subject.SubjectCode, subject.SubjectName, teaches.GradeID, gradeLevel.Year, gradeLevel.Number, teaches.TeachHour
            FROM teacher
    ...
    let mapSubject = resToJson.map((item:any) => ({GradeID : item.GradeID, SubjectID : item.SubjectID, SubjectCode : item.SubjectCode, SubjectName: item.SubjectName, TeachHour : item.TeachHour}))
    ...
    let createGradeObj = removeDulpicateGrade.map((year: any) => ({ //ใช้เก็บ Object ของชั้นเรียนและห้องเรียน
        ...
        ClassRooms: //เก็บห้องเรียนเป็น array (ClassRoom : [])
        removeDulpicateClassRooms.map((room: any) => //map array แยกห้องเรียนไปหาแต่ละชั้น
            year == room[0] ? {
                GradeID : parseInt(room),
                Subjects : mapSubject.filter((item) => item.GradeID == room).map((item) => ({SubjectID : item.SubjectID, SubjectCode : item.SubjectCode, SubjectName: item.SubjectName, TeachHour : item.TeachHour}))
