...
export async function GET(request: NextRequest) {
  ...
  const [teacher_responsibility, teacher_responsibilityField] =
    await pool.promise()
      .query(`SELECT teacher.TeacherID, subject.SubjectCode, subject.SubjectName, teaches.GradeID
        FROM teacher
  ...
  let mapGrade = resToJson.map((item: any) => item.GradeID[0]); //map เอาตัวแรกของรหัสชั้นเรียน เช่น 604 เป็น 6 เอาค่านี้ไว้ใช้เป็นชั้นเรียน
  ...
  return Response.json({ Teacher: teacher, Grade: createGradeObj });
}
