...
function generatePreviewSlots(config: ConfigData): PreviewSlot[] {
  ...
  for (let period = 1; period <= config.TimeslotPerDay; period++) {
    ...
    const isMiniBreak = config.HasMinibreak && period === config.MiniBreak.SlotNumber;
      ...
      <Stack spacing={2}>
        ...
        <Alert severity="info" variant="outlined">
          <Stack direction="row" spacing={3}>
            ...
            <Typography variant="body2">
              <strong>รวม:</strong> {totalMinutes} นาที ({(totalMinutes / 60).toFixed(1)} ชั่วโมง)
            </Typography>
          </Stack>
        </Alert>
            ...
            return (
              <Box
                ...
                sx={{
                  ...
                  p: 1.5,
                  borderRadius: 1,
                  bgcolor: slot.type !== 'class' ? 'action.hover' : 'transparent',
                  border: 1,
                  borderColor: slot.type !== 'class' ? 'divider' : 'transparent',
                }}
              >
                {/* Period Number */}
                <Box
        ...
        <Stack direction="row" spacing={2} justifyContent="center">
          <Chip icon={<ScheduleIcon />} label="คาบเรียน" color="primary" size="small" />
          <Chip icon={<LunchIcon />} label="พักเที่ยง" color="warning" size="small" />
          <Chip icon={<CoffeeIcon />} label="พักเบรก" color="info" size="small" variant="outlined" />
        </Stack>
      </Stack>
    </Paper>
  );
