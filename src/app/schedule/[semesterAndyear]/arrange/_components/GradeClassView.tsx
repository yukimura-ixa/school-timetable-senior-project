...
function ClassCard({ classData }: { classData: ClassScheduleData }) {
  const completionPercentage = classData.totalPeriods > 0
  ...
  return (
    <Card
      ...
      sx={{
        height: '100%',
        border: '2px solid',
        borderColor: isComplete ? 'success.main' : isPartial ? 'warning.main' : 'divider',
        transition: 'all 0.2s',
        '&:hover': {
          boxShadow: 3,
                      ...
                      <Stack spacing={0.5}>
                        ...
                        <Stack direction="row" spacing={0.5} flexWrap="wrap" gap={0.5}>
                          <Chip
                            ...
                            label={teacher ? `${teacher.Prefix}${teacher.Firstname}` : 'ไม่ระบุครู'}
                            size="small"
                            variant="outlined"
...
export function GradeClassView({
  ...
  const completedClasses = classes.filter(c => c.assignedPeriods >= c.totalPeriods).length;
  ...
  const overallProgress = totalClasses > 0
    ? (classes.reduce((sum, c) => sum + c.assignedPeriods, 0) /
       classes.reduce((sum, c) => sum + c.totalPeriods, 0)) * 100
    : 0;
    ...
    <Box>
        ...
        <Stack spacing={2}>
          ...
          <Box>
            ...
            <LinearProgress
              ...
              color={overallProgress >= 100 ? 'success' : overallProgress > 0 ? 'warning' : 'inherit'}
      ...
      <Grid container spacing={2}>
        {classes.map(classData => (
          <Grid size={{ xs: 12, sm: 6, md: 4, lg: 3 }} key={classData.class.GradeID}>
