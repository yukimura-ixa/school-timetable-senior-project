...
export async function generateBatchPDF(
  ...
  for (let i = 0; i < elements.length; i += tablesPerPage) {
    const pageElements = elements.slice(
      i,
      Math.min(i + tablesPerPage, elements.length),
    );
    const pageTitles = titles.slice(
      i,
      Math.min(i + tablesPerPage, titles.length),
    );
    ...
    const tableHeight =
      (pageHeight - margin * 2 - 20 - (showSignatures ? 15 : 0)) /
      tablesPerPage;
    const spacing = 5;
    ...
    for (let j = 0; j < pageElements.length; j++) {
      ...
      const yOffset =
        margin + (headerText && i === 0 ? 15 : 0) + j * (tableHeight + spacing);
...
export async function generateTeacherBatchPDF(
  ...
  const titles = teacherNames.map(
    (name) =>
      `ตารางสอน${name ? `: ${name}` : ""} - ภาคเรียนที่ ${semester}/${academicYear}`,
  );
