...
export const ExportTeacherSummary = (
  ...
  academicYear: string,
    ...
    const unLockObject = (obj: Record<string, string>[], dataCount: number): Record<string, string> => {
      ...
      ...teachers.map((tch: teacher, index: number) => {
        const mapKey = timeSlotData.DayOfWeek.flatMap((day) =>
          timeSlotData.SlotAmount.map((num: number) => {
            ...
            const convertClass = filterClass.map((item: ClassScheduleWithSummary) => `${item.GradeID[0]}/${item.GradeID[2]}`).join(",")
            const firstClass = filterClass[0];
            const res = filterClass.length === 0 ? "" : firstClass?.IsLocked ? `${firstClass.SubjectCode}` : `${convertClass}`
            obj[keyname] =
