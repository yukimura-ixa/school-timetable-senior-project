...
export const ExportStudentTable = (
  ...
  for (let i = 0; i < grades.length; i++) {
    let grd = grades[i]; //นำข้อมูลของครูมาใช้ในแต่ละรอบ
    let filterClassDataByGID = []
    if (grades.length == 1){ //ถ้าส่งครูเข้ามาคนเดียว (หมายถึง พิมพ์ตารางรายคน)
      filterClassDataByGID = [...classData] //นำข้อมูลมาใช้ได้เลย
    }
    else{ //ถ้าเป็นตารางรวม ต้อง filter ก่อน
      filterClassDataByGID = classData.filter((item) =>
      item.GradeID.includes(grd.GradeID),
      );
    }
    timeSlotData = {
      ...timeSlotData,
      AllData: timeSlotData.AllData.map((item) =>
        filterClassDataByGID
          .map((item) => item.TimeslotID)
          .includes(item.TimeslotID)
          ? {
              ...item,
              subject: filterClassDataByGID.filter(
          ...
          ...timeSlotData.AllData.filter((item) => item.DayOfWeek == "MON").map(
            (item) =>
              item.Breaktime == "BREAK_BOTH"
                ...
                : Object.keys(item.subject).length == 0
                  ? ""
                  : item.subject.IsLocked ? "" : `${item.subject.teachers[0].Firstname}`,
          ),
        ];
  ...
  workbook.xlsx.writeBuffer().then((data) => {
    ...
    (anchor.href = url), (anchor.download = "ตารางนักเรียน.xlsx");
    anchor.click();
