...
import type { PublishReadinessResult, PublishReadinessStatus } from '../types/publish-readiness-types';
import { findIncompletGrades, type GradeCompletion } from '@/features/dashboard/domain/services/dashboard-stats.service';
import { validateProgramMOECredits, type ProgramValidationResult } from '@/features/program/domain/services/moe-validation.service';
 ...
 */
export function checkPublishReadiness(configData: FullConfigData): PublishReadinessResult {
  const issues: string[] = [];
  ...
  if (incompleteGrades.length > 0) {
    const incompleteMessages = incompleteGrades.map(g => 
  ...
  for (const program of configData.programs) {
    ...
    if (!result.isValid) {
      issues.push(`หลักสูตร ม.${program.Year} (${program.ProgramName}): ${result.errors.join(', ')}`);
    }
