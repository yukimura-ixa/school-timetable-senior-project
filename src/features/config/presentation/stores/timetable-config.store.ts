...
function validateConfigData(config: ConfigData): ValidationErrors {
  ...
  if (config.HasMinibreak) {
    ...
    if (config.MiniBreak.Duration < CONFIG_CONSTRAINTS.MINI_BREAK_DURATION.min) {
...
export const useConfigStore = create<ConfigStoreState>()(
  immer((set, get) => ({
    ...
    updateBreakSlot: (level, slot) => set((state) => {
    ...
    updateMiniBreak: (field, value) => set((state) => {
    ...
    resetToDefault: (academicYear, semester) => set((state) => {
      state.config = {
        ...DEFAULT_CONFIG,
      ...
      state.breakSlotOptions = generateTimeslotRange(DEFAULT_CONFIG.TimeslotPerDay);
    }),
...
export const useConfigErrors = () => useConfigStore((state) => state.validationErrors);
