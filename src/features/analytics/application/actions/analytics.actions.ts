 ...
 */
export const getGradeStats = cache(
  async (
    input: unknown,
  ): Promise<
    ActionResult<{
      ...
      gradeInfo: {
        ...
        Number: number;
        StudentCount: number;
        program: { ProgramName: string; ProgramCode: string } | null;
      } | null;
  ...
  > => {
    ...
    return handleAction(() =>
      overviewRepository.getGradeStats(
        validation.output.configId,
        validation.output.gradeId,
      ),
    );
  },
);
...
export const getProgramsWithMissingSubjects = cache(
  async (input: unknown): Promise<ActionResult<ProgramCompliance[]>> => {       
    ...
    return handleAction(() =>
      complianceRepository.getProgramsWithMissingSubjects(
        validation.output.configId,
      ),
    );
  },
