 ...
 */
async function getPeriodDistribution(configId: string): Promise<PeriodDistribution[]> {
  ...
  for (let p = 1; p <= maxPeriod; p++) {
    ...
    const utilizationRate = maxSlotsPerPeriod > 0
      ? (count / maxSlotsPerPeriod) * 100
      : 0
 ...
 */
async function getDayDistribution(configId: string): Promise<DayDistribution[]> {
  const config = parseConfigId(configId)
  ...
  return DAYS_OF_WEEK.map(dayInfo => {
    ...
    const utilizationRate = maxSlotsPerDay > 0
      ? (count / maxSlotsPerDay) * 100
      : 0
 ...
 */
async function getPeakHours(configId: string, limit = 3): Promise<PeriodDistribution[]> {
 ...
 */
async function getLeastUtilizedPeriods(configId: string, limit = 3): Promise<PeriodDistribution[]> {
  const distribution = await getPeriodDistribution(configId)
  return distribution
