...
export function useSubjectAssignment(
  ...
  const initialConfigs = useMemo(() => {
    ...
    subjects.forEach((s) => {
      const existing = program?.program_subject?.find(
        (ps) => ps.SubjectCode === s.SubjectCode,
      );
      configs[s.SubjectCode] = {
        ...
        isMandatory:
          existing?.IsMandatory ?? s.Category === SubjectCategory.CORE,
      };
    });
  ...
  const handleConfigChange = useCallback(
    (
      code: string,
      field: "minCredits" | "maxCredits" | "isMandatory",
    ...
    try {
      const selectedSubjects = Object.values(subjectConfigs)
        ...
        .map((config, i) => ({
          ...
          Category:
            subjects.find((s) => s.SubjectCode === config.SubjectCode)
      ...
      if (result.success && result.data) {
        ...
        if (moeValidation.isValid) {
          enqueueSnackbar("บันทึกสำเร็จ: ผ่านมาตรฐาน กระทรวงศึกษาธิการ", {
            variant: "success",
          });
        } else {
          enqueueSnackbar("บันทึกสำเร็จ แต่ไม่ผ่านมาตรฐาน กระทรวงศึกษาธิการ", {
            variant: "warning",
          });
        }
