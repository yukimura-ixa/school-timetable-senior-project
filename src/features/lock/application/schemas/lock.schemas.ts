...
export type GetLockedSchedulesInput = v.InferOutput<
  typeof getLockedSchedulesSchema
>;
 ...
 * Input: Array of ClassID integers
 */
export const deleteLocksSchema = v.pipe(
  v.array(
    v.pipe(
      v.number("ClassID ต้องเป็นตัวเลข"),
      v.integer("ClassID ต้องเป็นจำนวนเต็ม"),
    ),
    "ClassIDs ต้องเป็น array",
  ),
...
export const getLockTemplatesSchema = v.object({
  category: v.optional(
    v.picklist(
      ["lunch", "activity", "assembly", "exam", "other"],
      "ประเภทเทมเพลตไม่ถูกต้อง",
    ),
  ),
});

export type GetLockTemplatesInput = v.InferOutput<
  typeof getLockTemplatesSchema
>;
...
export type ApplyLockTemplateInput = v.InferOutput<
  typeof applyLockTemplateSchema
>;
