...
export function useConflictValidation(): ConflictValidationOperations {
   ...
   */
  const checkLockConflict = useCallback((timeslotID: string): boolean => {
   ...
   */
  const checkTeacherConflict = useCallback((timeslotID: string, teacherID: number): boolean => {
    if (!teacherID) return false;
    
    // Find timeslot details
    const timeslot = timeSlotData.AllData.find((slot) => slot.TimeslotID === timeslotID);
   ...
   */
  const checkRoomConflict = useCallback((timeslotID: string, roomID: number | null): boolean => {
  ...
  const checkTimeslotConflict = useCallback((
    ...
    if (currentTeacherID && checkTeacherConflict(timeslotID, parseInt(currentTeacherID))) {
      return {
        type: 'teacher',
  ...
  }, [checkLockConflict, checkTeacherConflict, checkRoomConflict, currentTeacherID]);

  /**
   * Check if timeslot is available for subject
   */
  const isTimeslotAvailable = useCallback((timeslotID: string, subject: SubjectData): boolean => {
   ...
   */
  const getConflictMessage = useCallback((timeslotID: string, subject: SubjectData): string => {
    const conflict = checkTimeslotConflict(timeslotID, subject);
