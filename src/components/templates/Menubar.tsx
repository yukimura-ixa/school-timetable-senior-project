              ...
              <React.Fragment key={item.id}>
                <div className="w-full flex flex-col relative">
                  ...
                  >
                    <div className="flex gap-5 justify-between w-full items-center">
                      <span className="flex items-center justify-start gap-5">
                        <item.IconStyle.Icon
                          className={`fill-[#4f4f4f] changetext`}
                        />
                        <p className="text-md text-[#4F4F4F] changetext">
                          {item.title}
                        </p>
                      </span>
                  ...
                  <div
                    ...
                    ${indexPoint === index && isOpen ? null : "hidden"}`}
                  >
                    {/* เทอม 1 */}
                    {`${item.link[0]}` === linkSelected ? (
                      <Link
                        onClick={() => setLinkSelected(`${item.link[0]}`)}
                        href={`${item.link[0]}`}
                        className="w-[165px] px-3 py-2 cursor-pointer border-r-8 border-cyan-600 bg-cyan-100 duration-500 semester-text"
                      >
                      ...
                      <Link
                        onClick={() => setLinkSelected(`${item.link[0]}`)}
                        href={`${item.link[0]}`}
                        className="w-[165px] px-3 py-2 cursor-pointer hover:bg-cyan-100 duration-500 semester-text"
                      >
                    ...
                    {`${item.link[1]}` === linkSelected ? (
                      <Link
                        onClick={() => setLinkSelected(`${item.link[1]}`)}
                        href={`${item.link[1]}`}
                        className="w-[165px] px-3 py-2 cursor-pointer border-r-8 border-cyan-600 bg-cyan-100 duration-500 semester-text"
                      >
