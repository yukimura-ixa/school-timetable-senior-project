          ...
          {scheduleMenu.map((item, index) => (
            <React.Fragment key={item.id}>
              <div className="w-full flex flex-col relative">
                ...
                ?
                <Link onClick={() => setLinkSelected(`${item.link[0]}`)} href={`${item.link[0]}`} className="w-[165px] px-3 py-2 cursor-pointer border-r-8 border-cyan-600 bg-cyan-100 duration-500 semester-text">
                <p className="text-md text-cyan-600">เทอม 1</p>
                </Link>
                :
                <Link onClick={() => setLinkSelected(`${item.link[0]}`)} href={`${item.link[0]}`} className="w-[165px] px-3 py-2 cursor-pointer hover:bg-cyan-100 duration-500 semester-text">
