...
/**
 * Enhanced room selection component with building/floor grouping
...
export default function RoomAutocomplete({
  ...
  // Find the option matching the current value
  const selectedOption = value
    ? options.find((opt) => opt.value === value.RoomID) || null
    : null;
  ...
  return (
    <Autocomplete
      ...
      onChange={(_, newValue) => {
        if (newValue) {
          const room = rooms.find((r) => r.RoomID === newValue.value);
          onChange(room || null);
        } else {
          onChange(null);
        }
