 ...
 * ```
 */
export async function getAllFeatureFlags(): Promise<Record<
  string,
  FlagConfig
> | null> {
  try {
...
export async function hasFeatureFlag(flag: FeatureFlag): Promise<boolean> {
  try {
    ...
    console.error(
      `[FeatureFlags] Error checking existence of flag "${flag}":`,
      error,
    );
    return false;
  }
...
export async function getFeatureFlagConfig(
  ...
  try {
    ...
    console.error(
      `[FeatureFlags] Error fetching config for flag "${flag}":`,
      error,
    );
    return null;
  }
